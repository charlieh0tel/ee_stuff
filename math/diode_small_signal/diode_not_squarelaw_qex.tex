...
That is why crystal radio builders and microwave detector designers reach for Ge and zero-bias Schottky diodes; see the zero-bias caveat below.
...
Near $\VQ = 0$---the zero-bias Schottky operating point---the normalized curvature is unchanged (the $-1$ in \eqref{eq:shockley} is a constant and contributes none), but the current scale collapses to $\IS$ and the junction resistance $n\VT/\IS$ becomes enormous; $\IS$, embedding impedance, and noise---not the boundaries above---govern the detector \cite{maas}.
