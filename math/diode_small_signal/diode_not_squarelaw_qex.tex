...
This is the basis of resistive RF mixers: the $\vd^2$ nonlinearity provides frequency translation.
...
Below the square-law range ($\vhd \lesssim 0.04\,n\VT \approx \SI{1}{\milli\volt}$ for $n \approx 1$), the diode's terminal current is linear to better than \SI{1}{\percent} and it behaves as a resistor $\rd = 1/\gd$---but the rectified and mixing products do not switch off. They continue to scale as $\vhd^{\,2}$, shrinking quadratically until they sink below the noise and drift floor of any practical detector. The boundary marks where the nonlinearity becomes measurable, not where it begins.
...
Mixing, rectification, and harmonic products are not zero, but they scale as $\vhd^{\,2}$ and sit far below practical noise floors.
...
HD2 grows from \SI{1}{\percent} to \SI{25}{\percent}; HD3 reaches \SI{4}{\percent} at the upper edge but is still small enough to neglect.
The two-term approximation \eqref{eq:quadratic} is valid, though least accurate at the upper edge, where the dropped cubic term adds about \SI{12}{\percent} to the fundamental and the predicted rectified DC runs about \SI{6}{\percent} low.
...
Second, the lower $\VQ$ of Ge and Schottky means the intermediate exponential range is narrow: the upper edge sits near \SI{91}{\milli\volt} for Ge (about \SI{64}{\milli\volt} beyond the square-law boundary), versus \SI{359}{\milli\volt} for a Si p--n diode (about \SI{311}{\milli\volt} of unclipped exponential operation). This explains the historical division of labor: Ge and zero-bias Schottky for sensitive weak-signal detection, where usable conductance at zero bias matters most, and Si p--n for switching and mixer service where the wider amplitude range is useful.

\section*{Practical consequences}

\textbf{Why Ge and Schottky beat Si for weak-signal detection.}
Everything in the framework scales with the conductance $\gd = \IQ/(n\VT)$, and a Ge or zero-bias Schottky diode has usable conductance at zero applied bias---Ge because of its large saturation current $\IS$, the Schottky because of its low barrier---so an unbiased detector already sits where rectification is efficient.
An unbiased Si p--n junction, with $\IS$ in the nanoamp range or below, has almost no conductance and is essentially deaf without a few hundred millivolts of bias.
The lower ideality factor of Ge and Schottky devices also nudges the square-law threshold $0.04\,n\VT$ down ($\sim$\SI{1.1}{\milli\volt} versus $\sim$\SI{1.9}{\milli\volt} for the Si p--n), but the zero-bias operating point is the decisive advantage.
This is the fundamental reason crystal radio builders and microwave detector designers reach for Ge and zero-bias Schottky diodes; see the zero-bias caveat below.

\textbf{Why bias helps.}
Biasing the diode at a small forward current $\IQ$ raises the conductance $\gd = \IQ/(n\VT)$ and hence the responsivity $\gd/(2n\VT)$ of the quadratic term, so a given junction swing produces more rectified output.
The square-law boundaries in junction voltage ($0.04\,n\VT$ and $n\VT$) do not move---they depend only on $n$ and $\VT$---but smaller signals now produce a measurable output, improving sensitivity at the cost of a bias circuit and quiescent current drain.
...
Near $\VQ = 0$---the operating point of zero-bias Schottky detectors---the normalized curvature is unchanged (the $-1$ in \eqref{eq:shockley} is a constant and contributes none of it), but the current scale collapses to $\IS$ and the zero-bias junction resistance $n\VT/\IS$ is enormous; detector behavior is set by $\IS$, the embedding impedance, and noise, and the $\VQ$-referenced boundaries above lose meaning \cite{maas}.
