...
The same Q-point Taylor expansion underlies both halves of this note --- applied at forward bias ($\VQ > 0$) in the first half and at reverse bias ($\VQ < 0$ in the sign convention of \eqref{eq:cj-raw}) in the second --- and produced two qualitatively distinct theories. Under forward bias, the exponential $I$--$V$'s curvature decomposes into a small-signal conductance $\gd$ plus a hierarchy of resistive nonlinearities, traversing four amplitude regimes as the drive grows: \emph{linear} ($\vhd \lesssim 0.04\,n\VT$, where $\gd$ alone dominates), \emph{quadratic / square-law} ($\vhd \lesssim n\VT$, where the $\vd^2$ term implements square-law mixing and HD2 reaches \SI{25}{\percent}), \emph{intermediate exponential} ($n\VT < \vhd \lesssim \VQ - 4n\VT$, where low-order truncation fails but the closed-form Bessel expansion captures rich unclipped harmonic content), and \emph{rectifying} ($\vhd \gtrsim \VQ - 4n\VT$, where the full Shockley equation is required). Across all four, the conversion occurs in a dissipative nonlinear conductance rather than a lossless reactance. Under reverse bias, the depletion capacitance's $(1 - \VD/V_0)^{-m}$ profile produces a structurally identical series in $\vhd/\phi_Q$ governed by a single amplitude limit $\vhd < |\VQ|$, but the harmonics are quadrature-phased to the drive and dissipate nothing on average; Manley--Rowe enforces lossless conversion as a constraint on the device's terminal power balance.
...
\bibitem{nxp1n4148}
Nexperia, \textit{1N4148; 1N4148W; 1N4448 --- High-speed diodes}, data sheet --- Si p--n signal diode; $n \approx 1.85$, $\VQ \approx \SI{0.55}{\volt}$ at $\IQ = \SI{100}{\micro\ampere}$ in Table~\ref{tab:diode-numbers}.

\bibitem{skyworks1n5711}
Skyworks Solutions, \textit{1N5711 --- Schottky Barrier Diode}, data sheet --- Si Schottky example with $\VQ \approx \SI{0.30}{\volt}$, $n \approx 1.08$ for Table~\ref{tab:diode-numbers}.

\bibitem{nxpbat54}
Nexperia, \textit{BAT54 series --- Schottky barrier diodes (BAT54, BAT54A, BAT54C, BAT54S)}, data sheet --- alternate Si Schottky for Table~\ref{tab:diode-numbers}; SOT-23 surface-mount equivalent of 1N5711.

\bibitem{1n34a}
NTE Electronics, \textit{NTE109 / 1N34A --- Germanium Signal Detector Diode}, data sheet --- Ge point-contact diode used for Table~\ref{tab:diode-numbers}; $\VQ \approx \SI{0.20}{\volt}$, $n \approx 1.05$ at $\IQ = \SI{100}{\micro\ampere}$. Numerical values are typical for Ge detector diodes; specific lots vary significantly.
