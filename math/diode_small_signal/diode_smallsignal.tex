...
The same Q-point Taylor expansion underlies both halves of this note --- applied at forward bias ($\VQ > 0$) in the first half and at reverse bias ($\VQ < 0$ in the sign convention of \eqref{eq:cj-raw}) in the second --- and produces two qualitatively distinct theories. Under forward bias, the exponential $I$--$V$'s curvature decomposes into a small-signal conductance $\gd$ plus a hierarchy of resistive nonlinearities, traversing four amplitude regimes as the drive grows: \emph{linear} ($\vhd \lesssim 0.04\,n\VT$, where $\gd$ alone dominates), \emph{quadratic / square-law} ($0.04\,n\VT \lesssim \vhd \lesssim n\VT$, where the $\vd^2$ term implements square-law mixing and HD2 reaches \SI{25}{\percent}), \emph{intermediate exponential} ($n\VT < \vhd \lesssim \VQ - 4n\VT$, where low-order truncation fails but the closed-form Bessel expansion captures rich unclipped harmonic content), and \emph{rectifying} ($\vhd \gtrsim \VQ - 4n\VT$, where the full Shockley equation is required). Across all four, the conversion occurs in a dissipative nonlinear conductance rather than a lossless reactance. Under reverse bias, the depletion capacitance's $(1 - \VD/V_0)^{-m}$ profile produces a structurally identical series in $\vhd/\phi_Q$ governed first by the forward-crossing limit $|\vhd| < |\VQ|$, with reverse breakdown imposing the additional practical bound in \eqref{eq:swing-combined}; the harmonics are quadrature-phased to the drive and dissipate nothing on average, and Manley--Rowe enforces lossless conversion as a constraint on the device's terminal power balance.

That bifurcation maps directly onto circuit topology. Resistive mixers exploit the forward-bias nonlinearity and accept a conversion-loss floor, typically several decibels, in exchange for broadband operation and simple embedding. Envelope detectors and rectifiers exploit the same resistive curvature or conduction asymmetry but are usually evaluated by sensitivity, detected-output error, or rectification efficiency rather than mixer conversion loss. Varactor multipliers, parametric amplifiers, and reactance-modulator upconverters exploit the reverse-bias nonlinearity and trade narrower bandwidth (set by the impedance match against $C_{jQ}$ and $r_s$), tighter swing tolerance, and the requirement for an external pump against the theoretical promise of \SI{100}{\percent} conversion --- or, in the three-frequency parametric case, conversion gain in excess of unity.

A real diode always exhibits both nonlinearities simultaneously; the operating bias and embedding impedances determine which one dominates the circuit response. Forward bias makes the resistive mechanism the larger contributor and the reactive one a parasitic; reverse bias inverts that ranking. At zero bias, especially in low-level Schottky detectors, the intended detection mechanism is still the resistive curvature of the full Shockley law about $\VD = 0$ (the regime boundaries derived earlier assumed $\VQ > 5n\VT$ for a robust forward bias and do not extend to this operating point; a zero-bias analysis Taylor-expands the full Shockley law about $\VD = 0$ rather than the forward-exponential approximation about $\VQ$, yielding its own threshold structure). The junction capacitance remains part of the RF embedding problem and often acts as a shunt parasitic rather than as the desired nonlinear element. The same physical device --- particularly a Schottky barrier diode --- can therefore serve either role depending on how the load impedances at the harmonics are arranged, so distinguishing the two analyses is a question of which Taylor expansion the embedding circuit allows to dominate, not of which physical effect is ``really'' present inside the junction.
