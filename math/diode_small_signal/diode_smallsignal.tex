...
\caption{Diode operating regimes by AC amplitude $\vhd$. Numerical values in parentheses are evaluated at $n \approx 1$ and $\VT \approx \SI{26}{\milli\volt}$ ($T = \SI{300}{\kelvin}$); both linear and square-law boundaries scale linearly with $n$, so a Si PN diode with $n=1.85$ has cutoffs roughly $1.9\times$ those tabulated here. See Table~\ref{tab:diode-numbers} for representative diode-specific values.}
...
\midrule
Linear small-signal &
$\vhd \lesssim 0.04\, n \VT$ \newline (${\sim}\SI{1}{\milli\volt}$ at $n\!\approx\!1$; HD2 $<$ \SI{1}{\percent}) &
$\id \approx \gd \vd$ &
Amplifier biasing, dynamic resistance $\rd = 1/\gd$. \\

Quadratic / square-law &
$\vhd \lesssim n \VT$ \newline (${\sim}\SI{26}{\milli\volt}$ at $n\!\approx\!1$) &
